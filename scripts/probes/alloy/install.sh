...
choose_install_action() {
  ...
  menu_option "4" "彻底清理残留" "删除配置、证书、数据及本脚本添加的软件源"
  menu_exit "0/q" "退出"
  while true; do
    local choice=""
...
choose_maintenance_action() {
  ...
  menu_option "5" "彻底清理" "永久删除配置、证书、数据及本脚本添加的软件源"
  menu_exit "0/q" "退出"
  while true; do
    local choice=""
...
choose_uninstall_mode() {
  ...
  menu_option "2" "彻底清理" "永久删除配置、证书、数据及本脚本添加的软件源"
  menu_exit "0/q" "取消并返回"
  while true; do
    local choice=""
    ...
    case "${choice}" in
      ...
      2)
        danger_card "彻底清理警告" \
          "将删除" "${purge_targets}" \
          "不可恢复" "${irreversible_items}" \
          "共享提醒" "Grafana 软件源可能同时供同机其他 Grafana 软件使用"
        PURGE_MODE=1
        return
        ;;
      0|q|Q)
...
validate_alloy_version() {
  local version="${1#v}"
  [[ "${version}" =~ ^[0-9]+\.[0-9]+\.[0-9]+([.-][0-9A-Za-z.-]+)?$ ]] || \
    die "无效的 ALLOY_VERSION：$1"
}
...
release_digest() {
  ...
  awk -v wanted="${asset_name}" '
    ...
    found && /"digest"[[:space:]]*:/ {
      ...
      sub(/".*$/, "", line)
      if (line ~ /^[0-9a-fA-F]+$/ && length(line) == 64) {
        print tolower(line)
        exit
      }
    }
...
release_version() {
  local metadata_file="$1"
  sed -n 's/.*"tag_name"[[:space:]]*:[[:space:]]*"v\{0,1\}\([^"[:space:]]*\)".*/\1/p' \
...
download_release_file() {
  ...
  if command -v curl >/dev/null 2>&1; then
    curl --fail --location --show-error --retry 3 --connect-timeout 15 \
      --proto '=https' --progress-bar --output "${destination}" "${url}"
...
install_official_package() {
  ...
  if [[ "${ALLOY_VERSION}" == "latest" ]]; then
    ...
    validate_alloy_version "${ALLOY_VERSION}"
...
uninstall_probe() {
  ...
  if [[ "${PURGE_MODE}" == "1" ]] && detect_interactive_device; then
    if ! ask_yes_no_default "${purge_confirmation}" no; then
      RETURN_TO_MAIN=1
