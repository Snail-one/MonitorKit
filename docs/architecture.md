...
node_exporter 使用 Prometheus 拉取模型：中心端探针清单保存在 `/etc/prometheus/probes/inventory.json`，每个启用目标渲染为独立的受管 `scrape_config`，因此可以为每台服务器设置不同地址、端口和 mTLS 文件。Alloy 使用主动推送模型，不生成抓取目标；每台 Alloy 将 `/etc/alloy/monitor.name` 中的唯一名称写入指标的 `instance`、`host` 标签及日志的 `host` 标签，界面同时读取 Prometheus/Loki 当前端口和安全状态作为安装参数提示。
