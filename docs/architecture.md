...
配置编辑通过受限的 `vim → nano → vi` 编辑器选择进入：Manager 在编辑期间保存原内容，编辑后调用组件自己的校验器。服务已在运行时才 reload/restart；未启动的服务保持停止，由服务管理菜单或 `monitorkit start|stop|restart` 手动控制。安装与更新只写入二进制、配置和 unit，执行 `daemon-reload`，不自动启动。用户执行“启动”时才 `systemctl enable --now`（开机自启并立即运行）。“停止”只 `systemctl stop`；“停止开机启动”执行 `systemctl disable`，不停止当前进程。失败修改即时清理，活动配置自动回滚。“重置配置”复用首次安装的默认模板重写主配置和 unit，再把受管探针写回 Prometheus 配置；独立开关文件与数据目录不重置。mTLS 与 Prometheus 远程写入分别使用受管状态标记生成组件 unit，因此二进制更新不会丢失独立开关状态。远程写入在 HTTPS/mTLS 和 HTTP 下都能独立启用；交互层会对 HTTP 明文传输显示风险说明并要求再次确认。Prometheus 指标保留期与磁盘上限保存在 `/etc/prometheus/storage.settings`，并写入 systemd unit 的 TSDB 参数；更新或改写 unit 时会读回该文件。Loki 日志保留期与摄入限制由配置菜单直接写入 `loki.yml` 的 `limits_config` 和 `compactor`；启用保留期时会打开 Compactor 删除，失败时回滚原配置。
